...
class DailyReplaySimulator {
  ...
  private boxInsertFromPole(): void {
    ...
    for (let r = 0; r < k; r += 1) {
      ...
      this.boxSetRaw(rr, col, this.cloneTorus(this.boxGetRaw(rr + 1, col)));
    }
...
export function verifyDailyReplayProof(
  proof: DailyReplayProof,
  ...
  const simulator = new DailyReplaySimulator(Math.trunc(proof.seed) >>> 0);
  let lastInputTime = -1;
