...
class ReplaySimulator {
  ...
  public stepOneTickAndCaptureGameOverSnapshot(): SimulationState | null {
    ...
    this.update();
    // update() may capture game-over state through a nested method call, which
    // TypeScript cannot infer after the explicit null reset above.
    const snapshot = this.gameOverSnapshot as SimulationState | null;
    if (!snapshot) {
      return null;
    }
    return {
